...
#        print(y_pred_stmt[:5])
  ...
  print('[PolySA Optimizer] II Mean Absolute Percentage Error (Geo. Mean): ' + str(gmean(II_mape)))
  print('[PolySA Optimizer] Depth Mean Absolute Percentage Error (Geo. Mean): ' + str(gmean(depth_mape)))
...
def predict_module_latency_xilinx(loop_struct, config):
  ...
  if "loop" in loop_struct:
    ...
    if user_expr.startswith('S_'):
      ...
      user_expr_prefix = user_expr.split('.')[0]
    stmt_name = config['module_name'] + '_' + loop_name + '_' + user_expr_prefix
    II = max(config['model_preds'][stmt_name + '_II'], 1)
    depth = max(config['model_preds'][stmt_name + '_depth'], 1)
...
def predict_kernel_latency(kernel, df, module_list, stmt_list, latency_info, verbose=0, cycle=5):
...
def xilinx_run(design_dir, kernel, train, predict, cycle, verbose):
  if predict:
    ...
    ret = optimizer.convert_latency_infos_to_df(latency_infos, design_infos)
    latency = predict_kernel_latency(kernel, ret['df'], ret['module_list'], \
        ret['stmt_list'], latency_info, verbose, cycle)
    return latency
