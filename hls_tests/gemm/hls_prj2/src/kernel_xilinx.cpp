        ...
        for (int c2 = 0; c2 <= 1; c2 += 1) {
          ...
          for (int c3 = 0; c3 <= 1; c3 += 1) {
            ...
            for (int c4 = 0; c4 <= 1; c4 += 1) {
              ...
              {
                A_t2 fifo_data;
                fifo_data = A[((4 * c0 + 2 * c3 + c4) * 8 + (4 * c2 + 2 * c5)) / 2];
...
{
    int p0 = idx; // module id
    A_t2 local_A_ping[2][2];
    A_t2 local_A_pong[2][2];
    bool arb = 0;
        ...
        for (int c2 = 0; c2 <= 1; c2 += 1) {
          ...
          for (int c3 = 0; c3 <= 1; c3 += 1) {
            ...
            for (int c4 = 0; c4 <= 1; c4 += 1) {
              ...
              {
                B_t2 fifo_data;
                fifo_data = B[((4 * c1 + 2 * c3 + c4) * 8 + (4 * c2 + 2 * c5)) / 2];
...
{
    int p0 = idx; // module id
    B_t2 local_B_ping[2][2];
    B_t2 local_B_pong[2][2];
    bool arb = 0;
    bool inter_trans_en = 1;
    bool intra_trans_en = 0;
    int c0, c0_prev;
              ...
              #pragma HLS PIPELINE II=1
                {
                  ...
                  for (int n = 0; n < 2; n++) {
                  #pragma HLS UNROLL
                      local_A[0][(0 / 2) * 2 + n] = Reinterpret<int>((ap_uint<32>)fifo_data(31, 0));
                ...
                }
                {
                  ...
                  for (int n = 0; n < 2; n++) {
                  #pragma HLS UNROLL
                      local_B[0][(0 / 2) * 2 + n] = Reinterpret<int>((ap_uint<32>)fifo_data(31, 0));
                      fifo_data = fifo_data >> 32;
                  }
                }
                // simd
                ...
                #pragma HLS UNROLL
                  ...
                  printf("PE A: %d B: %d\n", local_A[0][c8], local_B[0][c8]);
                }
                if (c2 == 1 && c5 == 1)
                  fifo_C_drain_out.write(local_C[c7][c6]);
                {
                  B_t2 fifo_data;
                  fifo_data = (Reinterpret<ap_uint<32> >(local_B[0][(0 / 2) * 2 + 1]), Reinterpret<ap_uint<32> >(local_B[0][(0 / 2) * 2 + 0]));
                  fifo_B_out.write(fifo_data);
                }
                {
                  A_t2 fifo_data;
                  fifo_data = (Reinterpret<ap_uint<32> >(local_A[0][(0 / 2) * 2 + 1]), Reinterpret<ap_uint<32> >(local_A[0][(0 / 2) * 2 + 0]));
                  fifo_A_out.write(fifo_data);
                }
              }
            }
