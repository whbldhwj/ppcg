  ...
  struct polysa_kernel_stmt *stmt)
{
  ...
  if (stmt->u.i.in) {
    ...
    if (stmt->u.i.filter_depth >= 0) {
      p = isl_printer_start_line(p);
      p = isl_printer_print_str(p, "if (c");
      p = isl_printer_print_int(p, stmt->u.i.filter_depth);
      p = isl_printer_print_str(p, " == p0) {");
      p = isl_printer_end_line(p);
      p = isl_printer_indent(p, 2);
      p = isl_printer_start_line(p);
      p = io_stmt_print_local_index(p, stmt);
  ...
  struct polysa_hw_module *module, int types)
{
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (!first)
      p = isl_printer_print_str(p, ", ");
    name = isl_space_get_dim_name(kernel->space, isl_dim_set, 1);
    if (types) {
      p = isl_printer_print_str(p, type);
      p = isl_printer_print_str(p, " ");
    }
  ...
  struct polysa_prog *prog, struct polysa_hw_module *module, struct hls_info *hls)
{
  ...
  p = isl_printer_to_file(prog->ctx, hls->kernel_h);
  p = isl_printer_set_output_format(p, ISL_FORMAT_C);
  p = print_module_header(p, prog, module); // TODO
  p = isl_printer_print_str(p, ";");
  p = isl_printer_end_line(p);
  isl_printer_free(p);
