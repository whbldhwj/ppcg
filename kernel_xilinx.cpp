...
void C_drain_IO_L3_out(C_t4 *C, hls::stream<C_t2> &fifo_C_drain_local_in)
{
    ...
    for (int c3 = 0; c3 <= 3; c3 += 1)
      for (int c4 = 0; c4 <= 3; c4 += 1) {
        ...
        {
          C_t4 fifo_data;
          fifo_data = local_C[c6][0];
          C[16*c3 + c4 + 4*c6] = fifo_data;
        }
        }
      }
}
